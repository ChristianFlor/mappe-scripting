...
build {
  ...
  provisioner "file" {
    source      = "provision/todos_api.sh"
    destination = "/tmp/todos_api.sh"
  }

  provisioner "remote-exec" {
    inline = [
      "chmod +x /tmp/todos_api.sh",
      "/tmp/todos_api.sh"
    ]
  }
