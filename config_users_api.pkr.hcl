...
build {
  ...
  provisioner "file" {
    source      = "provision/users_api.sh"
    destination = "/tmp/users_api.sh"
  }

  provisioner "remote-exec" {
    inline = [
      "chmod +x /tmp/users_api.sh",
      "/tmp/users_api.sh"
    ]
  }
